...
class Car extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            formErrors: { carType: '', carFuelDropDown: '', carAverageCatalogueCityCombusiton: 0, carAverageCatalogueRouteCombusiton: 0 },
            friendlyNames: { carType: 'Pole typ/marka samochodu', carFuelDropDown: 'Pole wybierz paliwo', carAverageCatalogueCityCombusiton: 'Pole średnie spalanie w mieście', carAverageCatalogueRouteCombusiton: 'Pole średnie spalanie w trasie' },
            carTypeValid: '',
    ...
    addItem(e) {
        ...
        if (fuelKey === "") {
            ...
            var itemToAdd = {
                ...
                carFuelKey: fuelKey,
                carAverageCatalogueCityCombusiton: this._inputElementCityCombustion.value,
                carAverageCatalogueRouteCombusiton: this._inputElementRouteCombustion.value,
    ...
    validateField(fieldName, value) {
        ...
        let carAverageCatalogueCityCombusitonValid = this.state.carAverageCatalogueCityCombusitonValid;
        let carAverageCatalogueRouteCombusitonValid = this.state.carAverageCatalogueRouteCombusitonValid;
        ...
        switch (fieldName) {
            case 'carType':
                ...
                fieldValidationErrors.carType = carTypeValid ? '' : 'jest za krótkie (przynajmniej 3 znaki)!';
                if (this.props.cars.filter(data => (data.carType.toLowerCase() === value.toLowerCase())).length > 0) {
                    carTypeValid = false;
                    fieldValidationErrors.carType = 'zawiera paliwo, które już zostało dodane, wprowadź inna nazwę.'
                }
                break;
            case 'carAverageCatalogueCityCombusiton':
                ...
                fieldValidationErrors.carAverageCatalogueCityCombusiton = carAverageCatalogueCityCombusitonValid ? '' : 'może zawierać tylko liczby! (Mogą być zmiennoprzecinkowe).';
                break;
            case 'carAverageCatalogueRouteCombusiton':
                ...
                fieldValidationErrors.carAverageCatalogueRouteCombusiton = carAverageCatalogueRouteCombusitonValid ? '' : 'może zawierać tylko liczby! (Mogą być zmiennoprzecinkowe).';
                break;
            case 'clean':
                carTypeValid = ''
                fieldValidationErrors.carType = ''
    ...
    validateForm() {
        this.setState(
            { formValid: this.state.carTypeValid && this.state.carAverageCatalogueCityCombusiton });
    }
        ...
        cars.forEach(function (carItem) {
            ...
            combustionReports.forEach(function (item) {
                if (carItem.key === item.carKey) {
                    combustionVelocityData.push([item.averageSpeed, item.averageCombusiton]);
                }
            });

            var data = combustionVelocityData;
            var resultCombustionVelocityData = regression.polynomial(data, { order: 2 }); //degree of polynomial

            var combustionRouteData = []
            combustionRouteData.push([0, 0])
            combustionReports.forEach(function (item) {
                if (carItem.key === item.carKey) {
                    combustionRouteData.push([item.routeKilometers, item.averageCombusiton]);
                }
            });

            var dataRoute = combustionRouteData;
            var resultCombustionRouteData = regression.polynomial(dataRoute, { order: 2 }); //degree of polynomial
                ...
                <div className="header">
                    <form role="form" onSubmit={this.addItem}>
                        <div className="form-group">
                            <input
                                name="carType"
                                className={`form-control ${this.errorFuelTypeClass(this.state.carTypeValid)}`}
                        ...
                        <div className="form-group">
                            <input
                                ...
                                className={`form-control ${this.errorFuelTypeClass(this.state.carAverageCatalogueCityCombusitonValid)}`}
                        ...
                        <div className="form-group">
                            <input
                                ...
                                className={`form-control ${this.errorFuelTypeClass(this.state.carAverageCatalogueRouteCombusitonValid)}`}
