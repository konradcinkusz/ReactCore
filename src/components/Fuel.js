...
class Fuel extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            friendlyNames: { fuelType: 'Pole typ paliwa', fuelPrice: 'Pole cena paliwa' },
            fuelTypeValid: '',
            fuelPriceValid: '',
            formValid: false
        }
    ...
    handleUserInput(e) { this.setEventState(e, e.target.value); }
    ...
    validateField(fieldName, value) {
        ...
        switch (fieldName) {
            case 'fuelType':
                ...
                fieldValidationErrors.fuelType = fuelTypeValid ? '' : 'jest za krótkie (przynajmniej 3 znaki)!';
                if (this.props.fuelItems.fuels.filter(data => (data.fuelType.toLowerCase() === value.toLowerCase())).length > 0) {
                    fuelTypeValid = false;
                    fieldValidationErrors.fuelType = 'zawiera paliwo, które już zostało dodane, wprowadź inna nazwę.'
                }
                break;
            case 'fuelPrice':
                var str = new String(value);
                ...
                fieldValidationErrors.fuelPrice = fuelPriceValid ? '' : 'może zawierać tylko liczby! (Mogą być zmiennoprzecinkowe).';
                break;
            case 'clean':
                fuelTypeValid = ''
                fieldValidationErrors.fuelType = ''
                ...
                <div className="header">
                    <form role="form" onSubmit={this.addItem} >
                        <div className="form-group">
                            <input
                                name="fuelType"
                                className={`form-control ${this.errorFuelTypeClass(this.state.fuelTypeValid)}`}
                        ...
                        <div className="form-group">
                            <input
                                name="fuelPrice"
                                className={`form-control ${this.errorFuelTypeClass(this.state.fuelPriceValid)}`}
