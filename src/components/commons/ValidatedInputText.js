...
class ValidatedInputText extends Component {
    ...
    validateField(fieldName, value) {
        ...
        switch (fieldName) {
            case this.props.name:
                ...
                fieldValidationErrors.inputElement = inputElementValid ? '' : 'jest za krótkie (przynajmniej 3 znaki)!';
                //Ciekawe co się stanie, jak się nie poda nic w ogóle
                if (this.props.entries.filter(data => (data.names.toLowerCase() === value.toLowerCase())).length > 0) {
                    inputElementValid = false;
                    fieldValidationErrors.inputElement = 'zawiera element, który już został dodany, wprowadź inna nazwę.'
                }
                break;
            case 'clean':
                inputElementValid = ''
    ...
    validateForm() {
        this.setState(
            { formValid: this.state.inputElementValid });
    }
    ...
    render() {
        return (
            <div className="row">
                ...
                <Tooltip id={`${this.props.name}Tooltip`} text={this.props.inputTooltip} />
            </div>
        )
    }
}
